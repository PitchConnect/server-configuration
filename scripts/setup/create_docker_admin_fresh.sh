#!/bin/bash
# Fresh start docker-admin user creation script for macOS
# This script removes any existing docker-admin user and creates a new one
# Usage: sudo ./create_docker_admin_fresh.sh [ssh_key_file]
...
echo -e "${GREEN}Starting fresh docker-admin user setup...${NC}"

# Check if user already exists and remove it
if dscl . -read /Users/$USERNAME &> /dev/null; then
  echo -e "${YELLOW}User $USERNAME already exists. Removing completely...${NC}"
  
  # Kill any processes owned by the user
  echo -e "${YELLOW}Killing any processes owned by $USERNAME...${NC}"
  pkill -u $(id -u $USERNAME) 2>/dev/null || true
  
  # Remove from admin group
  echo -e "${YELLOW}Removing $USERNAME from admin group...${NC}"
  dseditgroup -o edit -d $USERNAME -t user admin 2>/dev/null || true
  
  # Remove from automatic login if configured
  if grep -q "$USERNAME" /Library/Preferences/com.apple.loginwindow.plist 2>/dev/null; then
