#!/bin/bash
# Reliable non-interactive script to create a docker-admin user on macOS
# This script should be run with sudo privileges
# Usage: sudo ./create_docker_admin_reliable.sh [ssh_key_file]
...
echo -e "${GREEN}Starting reliable docker-admin user setup...${NC}"

# Check if user already exists
if dscl . -read /Users/$USERNAME &> /dev/null; then
  echo -e "${YELLOW}User $USERNAME already exists. Repairing configuration...${NC}"
  
  # Get current UID
  CURRENT_UID=$(dscl . -read /Users/$USERNAME UniqueID | awk '{print $2}')
  if [ "$CURRENT_UID" -lt 500 ]; then
    echo -e "${YELLOW}Setting UID to 501 (was $CURRENT_UID)...${NC}"
    dscl . -change /Users/$USERNAME UniqueID "$CURRENT_UID" 501
  fi
  
  # Ensure user has proper attributes for login window visibility
  echo -e "${YELLOW}Setting critical user attributes...${NC}"
  dscl . -delete /Users/$USERNAME IsHidden 2>/dev/null || true
