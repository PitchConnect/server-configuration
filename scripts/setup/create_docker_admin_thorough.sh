#!/bin/bash
# Thorough docker-admin user creation script for macOS
# This script completely removes any existing docker-admin user using multiple methods
# Usage: sudo ./create_docker_admin_thorough.sh [ssh_key_file]
...
USER_ID=501  # Using 501 ensures visibility in login window
...
echo -e "${GREEN}Starting thorough docker-admin user setup...${NC}"

# Super thorough user removal
if id "$USERNAME" &>/dev/null || dscl . -read /Users/$USERNAME &>/dev/null; then
  echo -e "${YELLOW}User $USERNAME exists. Performing thorough removal...${NC}"
  ...
  # Remove from automatic login if configured
  echo -e "${YELLOW}Removing automatic login configuration...${NC}"
  defaults delete /Library/Preferences/com.apple.loginwindow autoLoginUser 2>/dev/null || true
  rm -f /etc/kcpassword 2>/dev/null || true
  ...
  # Multiple methods to delete the user
  echo -e "${YELLOW}Deleting user using multiple methods...${NC}"
  
  # Method 1: dscl
  dscl . -delete /Users/$USERNAME 2>/dev/null || true
  
  # Method 2: sysadminctl (macOS 10.13+)
  sysadminctl -deleteUser $USERNAME 2>/dev/null || true
  
  # Method 3: Remove from NetInfo database (older macOS)
  nicl . -delete /users/$USERNAME 2>/dev/null || true
  
  # Method 4: Remove specific user attributes
  for ATTR in $(dscl . -read /Users/$USERNAME 2>/dev/null | awk '{print $1}' | grep -v '^$'); do
    dscl . -delete /Users/$USERNAME $ATTR 2>/dev/null || true
  done
  
  # Method 5: Force remove from directory services cache
  dscacheutil -flushcache
  killall opendirectoryd 2>/dev/null || true
  sleep 2
  
  echo -e "${GREEN}User $USERNAME thoroughly removed.${NC}"
  
  # Final check
  if dscl . -read /Users/$USERNAME &>/dev/null; then
    echo -e "${RED}Warning: User $USERNAME still exists in directory services.${NC}"
    echo -e "${RED}Proceeding anyway, but this might cause issues.${NC}"
  else
    echo -e "${GREEN}Confirmed: User $USERNAME no longer exists in directory services.${NC}"
  fi
else
  echo -e "${GREEN}User $USERNAME does not exist. Proceeding with creation.${NC}"
fi

# Wait a moment for directory services to stabilize
sleep 3

# Create the user with proper attributes
echo -e "${GREEN}Creating fresh user $USERNAME...${NC}"
